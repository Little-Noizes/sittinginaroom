    ...
    def update_visualizations(self):
        ...
        if not self.audio_queue.empty():
            ...
            if len(data) > self.freq_analyzer.window_size:
                # Compute spectrogram with modified parameters
                frequencies, times, Sxx = signal.spectrogram(
                    data,
                    self.sample_rate,
                    nperseg=1024,  # Smaller window for better time resolution
                ...
                # Update spectrogram image with linear scaling
                self.spec_img.setImage(
                    Sxx_db,
                    scale=(times[-1]/Sxx_db.shape[1], 
                        (frequencies[-1] - frequencies[1])/Sxx_db.shape[0]),
                    pos=(0, frequencies[1])
                )
    
                # Auto range the view on the first update
                if self.iteration_count == 0:
                    self.spec_plot.getViewBox().autoRange()
