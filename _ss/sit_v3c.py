...
class AudioVisualizer(QMainWindow):
    def __init__(self):
        ...
        self.spectrogram_window = SpectrogramWindow(self.sample_rate)
        ...
        if self.input_device is not None and self.output_device is not None:
            try:
                def audio_callback(indata, outdata, frames, time, status):
                    ...
                    self.current_audio_chunk = current_data  # Store current data for spectrogram
    ...
    def update_visualizations(self):
        if not self.audio_queue.empty():
            ...
            if hasattr(self, 'spectrogram_window'):
                if hasattr(self, 'spectrogram_window') and hasattr(self, 'current_audio_chunk'):
                    # Update the spectrogram window with the latest chunk
                    self.spectrogram_window.update_spectrogram(self.current_audio_chunk)
...
class SpectrogramWindow(QWidget):
    def __init__(self, sample_rate, window_size=1024):
        super().__init__()
        ...
        self.spectrogram_data = np.zeros((100, window_size//2 + 1))  # Empty array for spectrogram
    ...
    def initialize_ui(self):
        ...
        # Create the spectrogram plot widget
        self.spectrogram_plot = pg.ImageView()
        layout.addWidget(self.spectrogram_plot)
        
        # Configure the spectrogram plot
        self.spectrogram_plot.setImage(self.spectrogram_data, autoLevels=False)
        self.spectrogram_plot.setLevels(-150, 150)  # dB scale range
        self.show()

    def update_spectrogram(self, audio_data):
        """Updates spectrogram data with the latest audio chunk."""
        # Make sure audio_data is the right size
        ...
        windowed_data = audio_data * np.hanning(self.window_size)
        spectrum = np.fft.rfft(windowed_data)
        magnitude = 20 * np.log10(np.maximum(np.abs(spectrum), 1e-10))  # Convert to dB

        # Shift the existing spectrogram data and append new spectrum
