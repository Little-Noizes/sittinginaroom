...
class AudioVisualizer(QMainWindow):
    ...
    def setup_ui(self):
        ...
        spec_widget = QWidget()
        spec_widget.setMinimumWidth(600)  # Set minimum width for spectrogram
            ...
            try:
                def audio_callback(indata, outdata, frames, time, status):
                    ...
                    if self.is_playing and len(self.playback_buffer) >= frames:
                        ...
                        if len(self.playback_buffer) < frames:  # Loop the playback
                            self.playback_buffer.extend(self.recording_buffer)
                            self.iteration_count += 1  # Increment counter on loop
        ...
        if not self.audio_queue.empty():
            ...
            if len(data) > self.freq_analyzer.window_size:
                ...
                self.spec_img.setImage(
                    ...
                    autoLevels=False,
                    levels=(Sxx_db.max()-60, Sxx_db.max())
                )
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_Space:
            if not self.is_recording:
                ...
                self.recording_buffer = []
                self.iteration_count = 0  # Reset counter when starting new recording
                self.iteration_label.setText("Iteration: 0")
            else:
                self.is_recording = False
                self.is_playing = True
                self.playback_buffer = self.recording_buffer.copy()
                self.iteration_count = 1  # Set to 1 when playback starts
